...
def TestWorker(configuration, logFile):
    ...
    net.load_state_dict(
        torch.load(os.path.join(configuration["WeightFolder"], "Weights.pkl"))
    )
    ...
    testLoss, testAccuracy, testPredictions = Evaluate(
        testLoader, net, lossFunction, "Test", 0, 0, mode="single"
    )
...
def Main(configuration):
    ...
    try:
        with open(
            os.path.join(configuration["SaveFolder"], "Log.txt"), mode="w"
        ) as logFile:
        ...
        # Log the source code
        shutil.copy(
            __file__, os.path.join(configuration["SaveFolder"], "TestScript.py")
        )
